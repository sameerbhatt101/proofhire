...
library CrossChainOrderTypes {
    /// @notice ASC's legacy ERC-7683-inspired cross-chain order.
    /// @dev This seven-field schema predates the May 2026 ERC-7683 resolver
    ///      redesign and intentionally omits the draft's orderDataType field.
    ///      orderData encodes a CrossChainIntent struct.
    struct CrossChainOrder {
        address originSettler;
        address user;
        uint256 nonce;
        uint256 originChainId;
        /// @dev Opening deadline enforced by the source settler before the
        ///      successful settlement transaction can be proved here.
        uint32 openDeadline;
        /// @dev Deadline for execution on the destination chain.
        uint32 fillDeadline;
        bytes orderData;
    }
    ...
    struct BatchCrossChainOrder {
        ...
        ASCBridgeTypes.EVMTokenAmount[] userInputs;
    ...
    struct TokenOutput {
        ...
        ASCBridgeTypes.EVMTokenAmount tokenAmount;
    ...
    ///   must be rejected.
    /// - `sourceProofRequirement` is reserved. The destination currently accepts
    ///   only bytes32(0); nonzero proof-policy identifiers require a future,
    ///   separately configured trust domain.
    /// @dev Production consumers should use the canonical intent returned by
    ///      IBridgeIntentDecoder rather than decoding untrusted orderData directly.
    struct CrossChainIntent {
        OrderAction action;
        uint256 sourceChainId;
        uint256 destinationChainId;
        /// @dev Source token and the exact amount the user is sending.
        ASCBridgeTypes.EVMTokenAmount sourceAmount;
        /// @dev Destination token and the minimum amount the recipient must receive.
        ASCBridgeTypes.EVMTokenAmount minDestinationAmount;
        address recipient;
